...
import { ellipsify } from '@tokengator/ui';
import { AccountUiFormBurn, AccountUiFormSend } from './account-ui-form-send';

export function AccountUiTokenActions({
  available,
  burn,
  close,
  mint,
          ...
          onClick={() =>
            modals.open({
              ...
              children: (
                <AccountUiFormSend
                  available={available}
                  send={(input) => send(input).then(() => modals.closeAll())}
                />
              ),
            })
          }
          ...
          onClick={() =>
            modals.open({
              ...
              children: (
                <AccountUiFormBurn
                  available={available}
                  burn={(input) => burn(input).then(() => modals.closeAll())}
                />
              ),
            })
          }
      ...
      {close && (
        <ActionIcon
          ...
          onClick={() => {
            if (
              !window.confirm(
                `Close account '${ellipsify(mint.toString(), 8)}'?`
              )
            )
              return;

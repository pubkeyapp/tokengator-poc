...
import { useVerifyPayment } from '../../../../data-access'

export function PresetUiAppPaymentVerify() {
  const mutation = useVerifyPayment()
            ...
            mutation
              ...
              .then((res) => {
                if (res.valid) {
                  ...
                  toastWarning(`User ${user.name} has no payment tokens`)
                }
              })
              .catch((err) => {
                toastError(`Failed to verify payment token: ${err.message}`)
                console.log(err)
              })
          }}
        >
